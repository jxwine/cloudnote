...
编辑 `server/.env`，把自己的邮箱填进去：

```ini
CLOUDNOTE_ADMINS=you@example.com
```

多个用逗号隔开。改完 `pm2 restart cloudnote`。

然后用这个邮箱注册/登录**网页版**，账号菜单里就会多出「后台管理」。桌面客户端里没有这个
入口——后台是运维用的，浏览器里开就行。

> 留空（默认）就是没有人能进后台，接口一律 403。

### 发布一个新版本

1. 本地打包：`cd app && VITE_CLOUDNOTE_SERVER=https://你的域名 npm run dist`
2. 打开网页版 → 账号菜单 → 后台管理 → 客户端版本
3. 选中 `app/release/云笔记 Setup x.y.z.exe`，版本号会自动从文件名里读出来
4. 写更新说明——这段会**原样**显示在用户的更新提示里，一行一条
5. 点发布

服务端边收边写盘边算 sha256，不会把整个包读进内存。客户端下次检查更新就能看到，
下载完会自己校验一遍 sha256。

⚠ **Nginx 必须改两个地方**，否则 80MB 的包传不上去：
...
`deploy/nginx-cloudnote.conf` 里已经写好了（`/api/admin/releases` 那一段），
按第五步整段粘贴就有。没改的话上传会返回 **413**。

### 回滚

后台把出问题的版本「下架」，再把上一个稳定版重新传一次即可——客户端取的是**最近发布**的
那一条，不是版本号最大的那条。已经升上去的客户端不会被降级（只往高版本更新）。
...
- `uploads/` —— 上传的图片
- `releases/` —— 上传的客户端安装包（丢了不影响数据，重新传一次就行）
