...
`app/src/renderer/src/lib/sync.ts`、`store.ts` 里跟保存和冲突相关的部分，
以及 `Editor.tsx` 里跟落库、标题结算相关的部分，改完**一定要跑这个**：

```bash
npm run test:sync          # 会先 build，再开两个真实浏览器窗口当两台设备
```
...
- 用户敲进去的内容，要么在主笔记、要么在冲突副本，**绝不能两头都找不到**
- 正文被别人的版本顶掉时，本端必须收到告知（横幅或 toast 都行）
- 不许出现内容一模一样的重复冲突副本
- 各端最终要收敛一致，且和服务端对得上

加新场景时复用这些断言，别另写一套期望值——它们挡住的是「换个操作顺序又开始丢字」。

这些场景每一个都对应真出过的 bug，不是想象出来的用例。判断测试有没有用的办法是
**把修复还原回去看它红不红**：这套测试第一版写完时全绿，但把核心修复退回旧写法居然还是全绿——
那时它是没用的。补上「让归档请求失败」的注入之后才抓得住。以后加场景也建议这么验一次。
